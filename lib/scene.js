...
function scene(svgRoot, layout) {
  ...
  function appendNode(nodeDescriptor) {
    ...
    // todo: need to store this somewhere to unsubscribe
    hammer(ui.element, {
      recognizers:[ [hammer.Pan, { threshold: 1 }]]
    }).on('panstart panmove panend', handlePan);
  }
  ...
  function handlePan(e) {
    ...
    if (e.type === 'panmove') {
      ...
      if (panStatus[model.id]) layout.pinNode(model, panStatus[model.id].isPinned);
    }
  }
